  ...
  const content = (
    <>
      <div className="absolute inset-0 overflow-hidden">
        ...
        {isGame && (
          <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full border border-white/50 bg-white/80 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-slate-800 shadow-sm backdrop-blur">
            <Play className="h-3.5 w-3.5" /> Play
          </span>
        )}
        <div className="absolute inset-x-2 bottom-2 rounded-lg bg-white/85 px-3 py-2.5 shadow-md backdrop-blur">
          <div className="flex items-center justify-between text-sm sm:text-base font-bold text-slate-900 leading-tight">
            <span className="line-clamp-1">{project.title}</span>
            {hasExternal && <ExternalLink className="h-4 w-4 text-orange-500" />}
          </div>
        </div>
      </div>
    </>
